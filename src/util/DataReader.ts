...
export const DataReader = {
  ...
  readFloat: function(data: any, n: any) {
  ...
  readInteger: function(data: any, n: any) {
    return data[n] + (data[n + 1] << 8) + (data[n + 2] << 16) + (data[n + 3] << 24);
  },

  readSignedShort: function(data: any, n: any) {
  ...
  readShort: function(data: any, n: any) {
    return data[n] + (data[n + 1] << 8);
  },

  readBinaryString: function(data: any, n: any, length: any) {
    let end = length + n;
    let str = '';
    for (let i = n; i != end; ++i) {
      if (data[i] > 0) {
